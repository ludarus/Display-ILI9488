...
// this is how the bits were mapped which is the same as in the C8051F580 which
...
// */ sbit CANEIE  = CAN0CN ^ 3;         /* CAN Module Error Interrupt Enable
// Bit */ sbit CANIF   = CAN0CN ^ 4;         /* CAN Module Interrupt Flag */
// sbit CANDAR  = CAN0CN ^ 5;         /* CAN Disable Automatic Retransmission
// bit */ sbit CANCCE  = CAN0CN ^ 6;         /* CAN Configuration Change Enable
// bit */ sbit CANTEST = CAN0CN ^ 7;         /* CAN Test Mode Enable bit */

// CAN:
global bool bCanOn = false;
global pdata BOOL bPendTxMsg[N_OUT_MSGS] = {
    false}; // xmission pending flag array
...
void CANTX() {
  ...
  if (bCanOn) {
    ...
    //;;	if (!BOFF) // for CAN STATUS RECOVERY: intrpt should be
    //re-enabled after BusOFF recovery
    if (!(CAN0STAT & BOFF_MASK))
      CANSIE = 1;

  } else // if (!bCanOn)
  {
    if (!CANINIT) // if CAN should be OFF but still in RUN mode after
                  // setup/config command(s) (except @ON!)
    {
...
void clearCanMsgObjects() {
  ...
  //;;	CAN0DATL = 0xFB;         // Set direction to WRITE all IF registers to
  //Msg Obj, incld mask and data
  CAN0IF1CML = 0xFB;
  for (i = 1; i < 33; i++) {
    ...
    //;;	CAN0ADR = IF1CMDRQST; // Write blank (reset) IF registers to
    //each msg obj
    //;;	CAN0DATL = i;
    CAN0IF1CRL = i;
  }
...
void initCanFilter(int8 iMsg) {
  ...
  //;;	CAN0DAT  = 0x00F0;     // Set to WRITE, and alter all Msg Obj, incl
  //mask,  except DATA
  CAN0IF1CM = 0x00F0;
  ...
  //;;	CAN0DAT  = 0x0000;		 // Set mask1 ID low word
  //(auto-increment)(not used for 11 bit ID's)
  CAN0IF1M1 = 0x0000;

  //;;	CAN0DAT  = CanFilters[iMsg] | 0xC000; // IF1 Mask2 (filter Mask or'ed
  //with MXtd and MDir
  ...
  // CAN0ADR= IF1ARB1;    			   // Point to arbitration
  // register (auto-increment)
  //;;	CAN0DAT  = 0x0000;		 // Set arbitration1 ID low word
  //(auto-increment) (not used for 11 bit ID's)
  CAN0IF1A1 = 0x0000;
  // CAN0ADR= IF1ARB2;    			   // Point to arbitration
  // register (auto-increment)
  //;;	CAN0DAT  = CanIds[iMsg] | 0x8000;	// If1 Arb. 2 = ID ored with the
  //MsgVal bit set
  CAN0IF1A2 = CanIds[iMsg] | 0x8000;

  // set the message control register

  //;;	CAN0DAT  = 0x1485;     			   // Msg Cntrl: set RxIE,
  //UseMask, EoB (auto-increment)
  CAN0IF1MC = 0x1485;
...
void sendCanMsg() {
  ...
  //
  //;;	CAN0ADR = IF1CMDMSK;	// Point to Command Mask 1
  //;;	CAN0DATL = 0xBF;		// Set to WRITE, & alter all Msg Obj
  //except MASK (or B7)
  CAN0IF1CML = 0xBF;
  ...
  //;;	CAN0ADR = IF1ARB2;		// Point to arbitration register bits
  //28-16 for 11 bit ID
  //;;	CAN0DATH = CanTxQ[0] >> 3 | 0xA0;	// Must shift the ID right 3,
  //If1 Reg 2 = (MSGVal, Xtd, Dir, ID26 - 16), set MsgVal, and Dir
  ...
  // message ctl reg:
  // NewDat,MsgLst,IntPnd,UMask,TxIe,RxIE,RmtEn,TxRqst,EOB,res,res,res,DLC 3-0

  //;;	CAN0DATH = 0x89;			// set the New Dat, TX Int.
  //request and TX request bit (TxRqst)
  //;;	CAN0DATL = 0x80 | CanTxQ[1]; 	// set the EOB then Or in the number of
  //bytes
  CAN0IF1MCH = 0x89;
  ...
  //
  for (iMsgNum = 0; iMsgNum < N_OUT_MSGS;
       iMsgNum++) // scan for free (non-pending) slot
  ...
  if (iMsgNum == N_OUT_MSGS) // none free (all slots are pending)
  {
    if (!nLastSentTxMsg) // none sent out (and set by ISR) since the above scan
      return;
    iMsgNum = nLastSentTxMsg - _1ST_OUT_MSG; // nLastSentTxMsg just sent out and
                                             // freed by ISR (iMsgNum=index)
  }
...
void CAN_ISR() using 3 //
{
  ...
  //    byte Interrupt_ID = CAN0IID;          // Read which message object
  //    caused
  ...
  {
    if (Interrupt_ID <= N_FILTERS) // if interrupt caused by filtered reception
                                   // (1<=nMsg<=N_FILTERS)
    {
      SaveCanMsg(Interrupt_ID); // resets intrpt pend bit while reading &
                                // passing msg obj to uart1
      //			CAN0STAT ^= RxOk;                  // clear the
      //TX OK for testing.lly
      CAN0STAT &= ~RxOk; // clear the RXOK
    } else if (Interrupt_ID >= _1ST_OUT_MSG &&
               Interrupt_ID <= LAST_OUT_MSG) // if xmission complete
                                             // (1stOutMsg<=nMsg<=last=32)
    {                                        //		reset intrpt pend bit:
      CAN0IF2CML = 0x1C; // Command Mask 2 Config for TX ctrl: CLEAR IntPnd +
                         // NewDat bits in msg ctrl (while reading it)
      CAN0IF2CRL = Interrupt_ID; // Command Request Reg. Reset Tx msg control in
                                 // Obj "Interrupt_ID"
      bPendTxMsg[(nLastSentTxMsg = Interrupt_ID) - _1ST_OUT_MSG] =
          false;         // reset logical pending flag and set last sent msg#
      CAN0STAT &= ~TxOk; // clear the TX OK
    }

    //		p3_0on(); DO CAN LED HERE
    //		Interrupt_ID = CAN0IID;	// read next pend message number from
    //CAN INTREG, if any TEST+++
    Interrupt_ID =
        CAN0IIDL; // read next pend message number from CAN INTREG, if any
  }

  // **	errH = CAN0IIDH; ** this was in the old Raymond code and possibly should
  // be here status intrpt &/or error handler
  errH = CAN0IIDH;
...
#define LEC_MASK 7 // Last Error Code (LEC) bit-mask in CAN status reg (CAN0STA)
  ...
  {                                        //		handle error status update
    ...
    if (status & 0x20)  // (EPASS)if recv error became passive (maxd out)
      bCanError = true; // set recv error passive indication (reset only next
                        // poer cycle)
    ...
    if (status & 0x80) // if BOff
    {
      ...
      if (byLEC == 5) // if in BOFF recovery
        ...
        //				CANSIE = 1; 	  // may never get here,
        //since no status intrpt till set again by main()
        CANSIE = 0; // may never get here, since no status intrpt till set again
                    // by main()
                    //		}
                    //		else
                    //		{
                    // p3_0on();
      //			CANSIE = 0; 	  // may never get here, since
      //no status intrpt till set again by main()
    }

    (byte) CAN0STAT =
        LEC_MASK; // set LEC to No-Change state (also reset RXOK+TXOK)
  }
  TEST_PIN = 0;
}
