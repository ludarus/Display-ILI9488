...
//+	0x00F0 << 5	 //AlarmCmd	equ	08ah Must be 8bh for testing !!! 5-5-98 Alarm output control command
...
//	0x00FF << 5	 //AlarmCmd	equ	08ah Must be 8bh for testing !!! 5-5-98 Alarm output control command
...
// 0x1FFC is the 11 bits of the identifier
	...
	0x1FFC,			//AlarmCmd	equ	08ah Must be 8bh for testing !!! 5-5-98 Alarm output control command
	0x1FFC			// this is a command which is not used by the display but prevents timeouts when no data is being displayed

};

const unsigned int code CanIds[] =	// acceptable ID's (used in Arbitration Register)
{
//-	0x0080 << 5, 	// InitDBCmd
...
//-	0x008A << 5		//AlarmCmd	equ	08ah Must be 8bh for testing !!! 5-5-98 Alarm output control command
...
//0x1080
//Shift the result right by two to get the final alignment of the address.
//0x0420h
//0x84 = 0x0420h
...
//	0x0420 << 2, 	// End Database download command
	...
	0x0450 << 2,	// 0x8A AlarmCmd	equ	08ah Must be 8bh for testing !!! 5-5-98 Alarm output control command
	0x0416 << 2
};
...
// In the C8051F047 (original code base used) the CAN0STAT register was bit accessable
// this is how the bits were mapped which is the same as in the C8051F580 which is not bit addressable
// Here is are the masks used to check each bit inthe CAN0STAT register.

#define BOFF_MASK 0x80			//sbit BOFF   = CAN0STA ^ 7;          /* Bus Off Status                  */
#define EWARN_MASK 0x40			//sbit EWARN  = CAN0STA ^ 6;          /* Warning Status                  */
#define EPASS_MASK 0x20			//sbit EPASS  = CAN0STA ^ 5;          /* Error Passive                   */
#define RXOK_MASK 0x10			//sbit RXOK   = CAN0STA ^ 4;          /* Received Message Successfully   */
#define TXOK_MASK 0x08			//sbit TXOK   = CAN0STA ^ 3;          /* Transmit a Message Successfully */
#define LEC2_MASK 0x04			//sbit LEC2   = CAN0STA ^ 2;          /* Last error code bit 2           */
#define LEC1_MASK 0x02			//sbit LEC1   = CAN0STA ^ 1;          /* Last error code bit 1           */
#define LEC0_MASK 0x01			//sbit LEC0   = CAN0STA ^ 0;          /* Last error code bit            */
